...
const Operations = ({operations,mandar}) => {
  return (
    <div>
    ...
    <div className="col-xs-9 col-sm-9">
      <div>{numbers.map((el, index) =><button key={index} className="col-xs-4 col-sm-4 btn btn-default" style={{marginBottom: '10px'}} onClick={evt => {mandar(evt.target.text)}}><span>{el}</span></button>)}</div>
    </div>
